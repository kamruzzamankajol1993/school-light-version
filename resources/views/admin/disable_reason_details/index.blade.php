...
@section('title')
Student Disable Reason  List | {{ $ins_name }}
@endsection
...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-flex align-items-center justify-content-between">
            <h4 class="mb-0">Student Disable Reason List</h4>
...
<div class="row">
                        <div class="col-sm-6">
                            <div class="float-right d-md-block">
                                <div class="dropdown">
                                @if (Auth::guard('admin')->user()->can('disable_reason_add'))
<button class="btn btn-primary dropdown-toggle waves-effect  btn-sm waves-light" type="button" data-bs-toggle="modal" data-bs-target=".bs-example-modal-lg">
                                        <i class="far fa-calendar-plus  mr-2"></i> Add Student Disable Reason
                                    </button>
@endif
                                </div>
                            </div>
...
<div class="table-responsive">
    ...
    style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                    ...
                                    <th>Action</th>
                                        ...
                                        <tbody>
                                            @foreach ($disable_reason_details as $user)
  ...
  {{ $user->name }}
                                    ...
                                    <td>
                                      @if (Auth::guard('admin')->user()->can('disable_reason_update'))
    ...
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Update Student Disable Reason Information</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
        </button>
      </div>
                                                <div class="modal-body">
                                                    <form action="{{ route('admin.student_disable_reason.update') }}" method="POST" enctype="multipart/form-data">
...
@endif
                                  ...
                                  @if (Auth::guard('admin')->user()->can('disable_reason_delete'))
...
<button   type="button" class="btn btn-danger waves-light waves-effect  btn-sm" onclick="deleteTag({{ $user->id}})" data-toggle="tooltip" title="Delete"><i class="fas fa-trash-alt"></i></button>
                    <form id="delete-form-{{ $user->id }}" action="{{ route('admin.student_disable_reason.delete',$user->id) }}" method="POST" style="display: none;">
                      @method('DELETE')
                                                    @csrf
...
<div class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="myLargeModalLabel">Add  Student Disable Reason</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                </button>
            </div>
                        <div class="modal-body">
                            <form class="custom-validation" action="{{ route('admin.student_disable_reason.store') }}" method="post" enctype="multipart/form-data">
                              @csrf
                                <div class="row">
