...
<div class="row">
                        ...
                        <div class="col-sm-6">
                            <div class="float-right d-md-block">
                                <div class="dropdown">
                                @if (Auth::guard('admin')->user()->can('topic_create'))
<button class="btn btn-primary dropdown-toggle waves-effect  btn-sm waves-light" type="button" data-bs-toggle="modal" data-bs-target=".bs-example-modal-lg">
                                        <i class="far fa-calendar-plus  mr-2"></i> Add New Topic
                                    </button>
@endif
                                </div>
                            </div>
...
<div class="table-responsive">
    ...
    style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                   ...
                                   <th>Action</th>
                                        ...
                                        <tbody>
                                            @foreach ($topic_details as $key=>$user)
...
{{ $user->lesson_id }}
                                                ...
                                                $lesson_list =   DB::table('topic_lists')
                                                ->where('topic_table_id',$user->id)
                                                ->get();
                                                                                        ?>
...
<li>{{ $lesson_list_new->topic_name }}</li>
@endforeach
                                                                                  </td>
                                                                                  <td>

                                                                                    @if (Auth::guard('admin')->user()->can('topic_update'))
                                                ...
                                                <div class="modal-content">
                                                  <div class="modal-header">
                                                    <h5 class="modal-title" id="exampleModalLabel">Update Topic Information</h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                                                    </button>
                                                  </div>
                                                                                          <div class="modal-body">
                                                                                              <form action="{{ route('admin.topic.update') }}" method="POST" enctype="multipart/form-data">
                                                             ...
                                                             <option value="{{ $user_class_update->id }}" {{ $user->subject_id == $user_class_update->id  ? 'selected' : '' }} >{{ $user_class_update->name }}</option>
                                                                                ...
                                                                                @endforeach
                                                                                                    ...
                                                                                                    <div class="form-group col-md-12 col-sm-12">
                                                                                                        <label for="password">Lesson Name</label>
                                                                                            <select name="lesson_id"  class="form-control form-control-sm">

                                                                                                @foreach ($lessonlist_details as $user_class_update)
                                                                                                <option value="{{ $user_class_update->lesson_name }}" {{ $user->lesson_id == $user_class_update->lesson_name  ? 'selected' : '' }} >{{ $user_class_update->lesson_name }}</option>
                                                                                                        ...
                                                                                                        <table class="table table-bordered" id="dynamic_field">
                                                                                                            ...
                                                                                                            <tr>
                                                                                                                <td>
                                                                                                                    ...
                                                                                                                    <input type="text" name="name[]" value="{{ $lesson_list_new->topic_name }}" placeholder="Enter your Name" class="form-control form-control-sm name_list" />
                                          ...
                                          @if (Auth::guard('admin')->user()->can('topic_delete'))
...
<button   type="button" class="btn btn-danger waves-light waves-effect  btn-sm" onclick="deleteTag({{ $user->id}})" data-toggle="tooltip" title="Delete"><i class="fas fa-trash-alt"></i></button>
                    <form id="delete-form-{{ $user->id }}" action="{{ route('admin.topic.delete',$user->id) }}" method="POST" style="display: none;">
                      @method('DELETE')
                                                    @csrf
...
<div class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="myLargeModalLabel">Add New Topic</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                </button>
            </div>
                        <div class="modal-body">
                            <form class="custom-validation" action="{{ route('admin.topic.store') }}" method="post" enctype="multipart/form-data">
                              @csrf
                                <div class="row">
                ...
                <div class="form-group col-md-6 col-sm-12">
                                                    ...
                                                    <select name="lesson_id"  class="form-control form-control-sm">
                                                            ...
                                                            <table class="table table-bordered" id="dynamicAddRemove">
                                                                ...
                                                                <tr>
                                                                    <td><input type="text" id="first0" name="topic[]" placeholder="Enter Topic Name" class="form-control form-control-sm" />
                                                                    </td>
                                                                    <td><button type="button" name="add" id="dynamic-ar" class="btn btn-outline-primary"><i class="uil-plus"></i></button></td>
                                                                </tr>
                                                            </table>
                                                            </div>
                                                        </div>
...
<script type="text/javascript">
    $("select[name='subject_id']").change(function(){
        ...
        var token = $("input[name='_token']").val();
        $.ajax({
            url: "{{ route('lesson_search_subject_wise') }}",
            method: 'POST',
            data: {class_id:class_id,department_id:department_id,section_id:section_id,subject_id:subject_id, _token:token},
            success: function(data) {
              $("select[name='lesson_id'").html('');
              $("select[name='lesson_id'").html(data.options);
            }
        });
    });
  </script>
  ...
  <script type="text/javascript">
    ...
    $("#dynamic-ar").click(function () {
        ++i;
        $("#dynamicAddRemove").append('<tr><td><input type="text" id="first'+i+'" name="topic[]" placeholder="Enter Topic Name" class="form-control form-control-sm" /></td><td><button type="button" class="btn btn-outline-danger remove-input-field"><i class="uil-trash"></i></button></td></tr>'
            );
    });
